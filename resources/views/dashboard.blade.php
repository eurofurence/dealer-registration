    ...
    @if (isset($application))
        @if (
            $application->getStatus() === \App\Enums\ApplicationStatus::Open ||
                (($application->status === \App\Enums\ApplicationStatus::TableOffered ||
                    $application->status === \App\Enums\ApplicationStatus::Waiting) &&
                    !$application->is_notified))
            ...
            @else
                <div class="alert alert-info text-center">
                    ...
                    <p>Please wait for our team to process the application. The main account of your dealership will be notified via email if
                        the application status changes.</p>
                </div>
            @endif
        @elseif ($application->status === \App\Enums\ApplicationStatus::TableOffered && $application->is_notified)
            @if ($application->type === \App\Enums\ApplicationType::Dealer)
                <div class="alert alert-info text-center">
            ...
            @else
                <div class="alert alert-info text-center">
                    <h3>Congratulations!</h3>
                    <p>The application of the dealership you are part of was accepted! The main account of your dealership has been informed via email and needs to review and accept the assigned table.</p>
                    <p>For any questions about the table your dealership was assigned, please contact the person who initated the application.</p>
                </div>
            @endif
        @elseif ($application->status === \App\Enums\ApplicationStatus::TableAccepted)
            <div class="alert alert-info text-center">
                <h3>See you at Eurofurence!</h3>
                <h5>Your table in this year's Dealers' Den will be: <strong>{{ $application->table_number }}</strong></h5>
                @switch($efRegistrationStatus)
                    @case('new')
                    @case('approved')

                    @case('partially paid')
                        <p>Our records show that your EF registration status is <em>{{ $efRegistrationStatus }}</em>. Please check
                            and make sure to settle all outstanding dues in a timely manner to ensure everything runs smoothly when
                            you arrive.</p>
                    @break

                    @case('paid')
                        <p>According to our records, your EF registration status is <em>{{ $efRegistrationStatus }}</em>, and you
                            are all
                    ...
                    @case('cancelled')
                        <p>Going by our records, it seems your EF registration status is <em>{{ $efRegistrationStatus }}</em>.
                            Please contact us at dealers@eurofurence.org if you are not planning on attending Eurofurence this year
                            after all!</p>
                    @break
